...
def build_chain():
  """ A chain which generates for answer for a dialogue between 
  customer and technician from the kendra context or from llm knowledge
  """
...
def run_chain(chain, convo: str, history=[]):
  """to run the chain with the qa chain and prompt_

  Args:
      chain (Any):  A conversational retrival chain to search for answers from context
      convo (str): the conversation between the customer and technician
      history (list, optional): _description_. Defaults to [].

  Returns:
      dict: contains the convo and answer generated by the LLM
  """

  return chain({"question": convo,"chat_history": history})
